...
// Enhanced Query Display Component for Table
const QueryPreview = ({ query, maxLength = 50 }) => {
  const [showFull, setShowFull] = useState(false);
  
  const truncatedQuery = query.length > maxLength ? query.substring(0, maxLength) + '...' : query;
  
  return (
    <div className="relative group">
      <div 
        className="font-mono text-xs cursor-pointer hover:bg-gray-700/50 p-2 rounded transition-colors"
        onClick={() => setShowFull(!showFull)}
      >
        <span className="text-red-400">{showFull ? query : truncatedQuery}</span>
      </div>
      
      {!showFull && query.length > maxLength && (
        <div className="absolute bottom-full left-0 mb-2 hidden group-hover:block z-10">
          <div className="bg-gray-900 border border-gray-600 rounded-lg p-3 shadow-lg max-w-md">
            <SQLHighlighter query={query} title="Full Query Preview" />
          </div>
        </div>
      )}
    </div>
...
// Advanced SQL Syntax Highlighter Component
const SQLHighlighter = ({ query, showLineNumbers = false, title = null }) => {
  const highlightSQL = (sql) => {
    // Enhanced patterns for better highlighting
...
// Real-time Attack Log Component
...
// Attack Analysis Chart Component
                    ...
                    {filteredAttacks.slice(0, 10).map((attack) => (
                      ...
                      >
                        ...
                        <td className="p-3 max-w-xs">
                          <QueryPreview query={attack.query} maxLength={30} />
                        </td>
                        <td className="p-3 text-gray-300">
                          {attack.attackType}
                        </td>
