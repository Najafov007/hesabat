...
// Simple Router Hook
const useRouter = () => {
  const [currentRoute, setCurrentRoute] = useState('/dashboard/overview');
...
const Sidebar = ({ isOpen, toggleSidebar, currentRoute, navigate }) => {
  ...
  const navItems = [
    { path: '/dashboard/overview', icon: Activity, label: 'Overview', color: 'text-blue-400' },
    { path: '/dashboard/ddos', icon: AlertTriangle, label: 'DDoS Attack', color: 'text-red-400' },
    { path: '/dashboard/sql-injection', icon: Database, label: 'SQL Injection', color: 'text-orange-400' },
    { path: '/dashboard/mitm', icon: Network, label: 'MITM Attack', color: 'text-purple-400' },
    { path: '/dashboard/malware-upload', icon: Upload, label: 'Malware Upload', color: 'text-yellow-400' },
    { path: '/logs/overview', icon: FileText, label: 'Security Logs', color: 'text-green-400' },
    { path: '/charts', icon: BarChart3, label: 'Analytics', color: 'text-cyan-400' }
  ];
            ...
            return (
              <button
                ...
                className={`w-full flex items-center space-x-3 p-3 rounded-lg transition-all duration-200 hover:scale-105 ${
                  isActive
                    ? `${isDark ? 'bg-gray-800 text-white' : 'bg-gray-100 text-gray-900'} shadow-md`
...
const Layout = ({ children, currentRoute, navigate }) => {
    ...
    <div className="space-y-6">
      ...
      <div className={`p-6 rounded-xl ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} border shadow-lg`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <button className="flex items-center space-x-2 p-3 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors">
            <AlertTriangle className="h-5 w-5" />
            <span>Start DDoS Sim</span>
          </button>
          <button className="flex items-center space-x-2 p-3 bg-orange-600 hover:bg-orange-700 text-white rounded-lg transition-colors">
            <Database className="h-5 w-5" />
            <span>SQL Injection</span>
          </button>
          <button className="flex items-center space-x-2 p-3 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-colors">
            <Network className="h-5 w-5" />
            <span>MITM Attack</span>
          </button>
          <button className="flex items-center space-x-2 p-3 bg-yellow-600 hover:bg-yellow-700 text-white rounded-lg transition-colors">
...
const DDosSimulation = () => {
  const { isDark } = useDarkMode();
  return (
    <div className={`${isDark ? 'text-white' : 'text-gray-900'} text-center py-20`}>
      <AlertTriangle className="h-16 w-16 text-red-400 mx-auto mb-4" />
      <h2 className="text-2xl font-bold mb-2">DDoS Attack Simulation</h2>
      <p className="text-gray-400">Coming in Part 2 - Live packet flooding with real-time charts</p>
    </div>
  );
};
...
const SqlInjectionSimulation = () => {
  ...
  return (
    <div className={`${isDark ? 'text-white' : 'text-gray-900'} text-center py-20`}>
      <Database className="h-16 w-16 text-orange-400 mx-auto mb-4" />
      <h2 className="text-2xl font-bold mb-2">SQL Injection Simulation</h2>
      <p className="text-gray-400">Coming in Part 3 - Malicious query analysis and syntax highlighting</p>
    </div>
  );
};
...
const MitmSimulation = () => {
  ...
  return (
    <div className={`${isDark ? 'text-white' : 'text-gray-900'} text-center py-20`}>
      ...
      <h2 className="text-2xl font-bold mb-2">Man-in-the-Middle Simulation</h2>
      <p className="text-gray-400">Coming in Part 4 - Packet interception and credential harvesting</p>
    </div>
  );
};
...
const MalwareUploadSimulation = () => {
  ...
  return (
    <div className={`${isDark ? 'text-white' : 'text-gray-900'} text-center py-20`}>
      ...
      <h2 className="text-2xl font-bold mb-2">Malware Upload Simulation</h2>
      <p className="text-gray-400">Coming in Part 5 - File scanning and threat detection</p>
    </div>
  );
};
...
const SecurityLogs = () => {
  ...
  return (
    <div className={`${isDark ? 'text-white' : 'text-gray-900'} text-center py-20`}>
      ...
      <h2 className="text-2xl font-bold mb-2">Security Logs</h2>
      <p className="text-gray-400">Coming in Part 6 - Advanced filtering and log analysis</p>
    </div>
  );
};
...
const Analytics = () => {
  ...
  return (
    <div className={`${isDark ? 'text-white' : 'text-gray-900'} text-center py-20`}>
      ...
      <p className="text-gray-400">Coming in Part 7 - Interactive charts and threat visualization</p>
    </div>
  );
};
...
const AppRouter = () => {
  ...
  const renderPage = () => {
    switch (currentRoute) {
      ...
      case '/dashboard/sql-injection':
        return <SqlInjectionSimulation />;
      case '/dashboard/mitm':
        return <MitmSimulation />;
      case '/dashboard/malware-upload':
        return <MalwareUploadSimulation />;
      case '/logs/overview':
        return <SecurityLogs />;
      case '/charts':
        return <Analytics />;
